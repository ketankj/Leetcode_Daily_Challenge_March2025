# You are given a positive integer days representing the total number of days an employee is available for work (starting from day 1). You are also given a 2D array meetings of size n where, meetings[i] = [start_i, end_i] represents the starting and ending days of meeting i (inclusive).

# Return the count of days when the employee is available for work but no meetings are scheduled.

# Note: The meetings may overlap.

 

# Example 1:

# Input: days = 10, meetings = [[5,7],[1,3],[9,10]]

# Output: 2

# Explanation:

# There is no meeting scheduled on the 4th and 8th days.

# Example 2:

# Input: days = 5, meetings = [[2,4],[1,3]]

# Output: 1

# Explanation:

# There is no meeting scheduled on the 5th day.

# Example 3:

# Input: days = 6, meetings = [[1,6]]

# Output: 0

# Explanation:

# Meetings are scheduled for all working days.

class Solution(object):
    def countDays(self, days, meetings):
        # Step 1: Sort the meetings by start day
        meetings.sort()

        merged_intervals = []
        
        for meeting in meetings:
            # If no merged intervals yet, or if the current meeting doesn't overlap with the last merged interval
            if not merged_intervals or merged_intervals[-1][1] < meeting[0]:
                merged_intervals.append(meeting)
            else:
                # Otherwise, merge the current meeting with the last merged interval
                merged_intervals[-1][1] = max(merged_intervals[-1][1], meeting[1])

        # Step 2: Count the total number of covered days
        covered_days = 0
        for start, end in merged_intervals:
            covered_days += (end - start + 1)

        # Step 3: Return the total days minus the covered days
        return days - covered_days

        


s = Solution()
days = 10
meetings = [[5,7],[1,3],[9,10]]
print(s.countDays(days,meetings))  
